...
describe("CLI Error Display Integration", function()
  ...
  describe("Enhanced Error Response Format Detection", function()
    it("should detect enhanced error response format vs simple format", function()
      ...
      assert.equals("enhanced", format_type, "Should detect enhanced response format")
    end)
    ...
    it("should detect simple error response format", function()
      ...
      assert.equals("simple", format_type, "Should detect simple response format")
    end)
  end)
  ...
  describe("Rich CLI Error Formatting", function()
    it("should format single tag resolution error with suggestions", function()
      ...
      local formatted_output = cli_utils.format_enhanced_error_response(enhanced_response)
      
      -- Expect rich formatting with symbols and suggestions
      assert.matches("✗", formatted_output, "Should contain error symbol")
      assert.matches("TAG RESOLUTION ERRORS:", formatted_output, "Should group errors by phase")
      assert.matches("editor.*Tag overflow", formatted_output, "Should show resource and error")
      assert.matches("Consider using absolute tag", formatted_output, "Should show suggestions")
      assert.matches("•", formatted_output, "Should format suggestions with bullets")
    end)
    ...
    it("should format partial success with mixed errors", function()
      ...
      local cli_utils = require("cli.response_formatter")
      local formatted_output = cli_utils.format_enhanced_error_response(partial_response)
      
      -- Expect both error and success sections
      assert.matches("SPAWNING ERRORS:", formatted_output, "Should show spawning errors section")
      assert.matches("✗.*browser.*Command not found", formatted_output, "Should show spawn error")
      assert.matches("PARTIAL SUCCESS:", formatted_output, "Should show partial success section")  
      assert.matches("✓.*editor.*PID.*12345", formatted_output, "Should show successful spawn")
      assert.matches("Check if.*installed", formatted_output, "Should show spawn error suggestions")
    end)
    ...
    it("should format multiple error types with grouping", function()
      ...
      local cli_utils = require("cli.response_formatter")
      local formatted_output = cli_utils.format_enhanced_error_response(multi_error_response)
      
      -- Expect grouped display
      assert.matches("TAG RESOLUTION ERRORS:", formatted_output, "Should group tag resolution errors")
      assert.matches("SPAWNING ERRORS:", formatted_output, "Should group spawning errors")
      
      -- Expect all errors shown
      assert.matches("✗.*editor.*Tag overflow", formatted_output, "Should show editor tag error")
      assert.matches("✗.*browser.*Invalid tag", formatted_output, "Should show browser tag error")
      assert.matches("✗.*terminal.*Permission denied", formatted_output, "Should show terminal spawn error")
    end)
  end)
  ...
  describe("Backwards Compatibility", function()
    it("should handle simple error responses with basic formatting", function()
      ...
      local cli_utils = require("cli.response_formatter")
      local formatted_output = cli_utils.format_simple_error_response(simple_response)
      
      -- Should use basic formatting for simple responses
      assert.matches("Failed to start.*test%-project", formatted_output, "Should show project failure")
      assert.matches("Tag resolution failed", formatted_output, "Should show error message")
      assert.matches("editor", formatted_output, "Should mention failed resource")
    end)
    ...
    it("should handle success responses with resource details", function()
      ...
      local cli_utils = require("cli.response_formatter")
      local formatted_output = cli_utils.format_success_response(success_response)
      
      -- Should show success details
      assert.matches("Started.*success%-test.*successfully", formatted_output, "Should show success message")
      assert.matches("Spawned.*2.*resources", formatted_output, "Should show spawn count")
      assert.matches("✓.*editor.*PID.*12345", formatted_output, "Should show editor details")
      assert.matches("✓.*browser.*PID.*12346", formatted_output, "Should show browser details")
    end)
  end)
  ...
  describe("CLI Integration with Handler", function()
    it("should process enhanced error responses end-to-end", function()
      ...
      assert.equals("COMPLETE_FAILURE", response.error_type, "Should be complete failure")
      assert.equals(1, #response.errors, "Should have one error")
      assert.equals("tag_resolution", response.errors[1].phase, "Should be tag resolution error")
      ...
      assert.matches("TAG RESOLUTION ERRORS:", formatted, "CLI should format with rich display")
      assert.matches("✗.*editor.*Tag overflow", formatted, "Should show detailed error")
      assert.matches("Consider using absolute tag", formatted, "Should show suggestions")
