...
function response_formatter.detect_response_format(response)
  ...
  if response.status == "success" or (response.spawned_resources and not response.error) then
    return "success"
  end
...
function response_formatter.format_enhanced_error_response(response)
  ...
  table.insert(sections, build_error_header(response.project_name, response.metadata))
  
  -- Format errors using rich formatter
  if response.errors and #response.errors > 0 then
    local formatted_errors = formatter.format_multiple_errors_for_cli(response.errors)
    table.insert(sections, formatted_errors)
  end
  
  -- Format partial success if present
  if response.partial_success then
    local formatted_success = formatter.format_partial_success_for_cli(response.partial_success)
...
function response_formatter.format_success_response(response)
  ...
  local total_spawned = response.total_spawned or (response.spawned_resources and #response.spawned_resources) or 0
